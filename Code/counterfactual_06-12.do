...
reg="minwage married union smsa partt nonwhite grade1to4 grade5to6 grade7to8 grade9 grade10 grade11 highschool somecollege college egrade1to4 egrade5to6 egrade7to8 egrade9 egrade10 egrade11 ehighschool esomecollege ecollege exper exper2 exper3 exper4 oc2-oc22 ind2-ind51"
reg1="married union smsa partt nonwhite grade1to4 grade5to6 grade7to8 grade9 grade10 grade11 highschool somecollege college egrade1to4 egrade5to6 egrade7to8 egrade9 egrade10 egrade11 ehighschool esomecollege ecollege exper exper2 exper3 exper4 oc2-oc22 ind2-ind51"
wei="eweight"
dep="wage"
...
while(i<=nys){
		...
		else coef1[.,i]=binary_reg(group1, y1:<=ys[i], reg, wei, method, idx)
		i=i+1
	}
...
reg="minwage ch05 married union smsa nonwhite grade5to6 grade7to8 grade9 grade10 grade11 highschool college egrade5to6 egrade7to8 egrade9 egrade10 egrade11 ehighschool ecollege exper exper2 exper3 exper4 oc2-oc22 ind2-ind51"
reg1="ch05 married union smsa nonwhite grade5to6 grade7to8 grade9 grade10 grade11 highschool college egrade5to6 egrade7to8 egrade9 egrade10 egrade11 ehighschool ecollege exper exper2 exper3 exper4 oc2-oc22 ind2-ind51"
wei="eweight"
dep="uhours"
y0=st_data(.,dep, group0)
y1=st_data(.,dep,group1)
...
if (gender=="men"){
	if (B=="Yes"){
		fh=fopen("${results}06-12/qwages","r")
		qwages=fgetmatrix(fh)
		fclose(fh)
		fh=fopen("${results}06-12/qhours","r")
		...
		fh=fopen("${results}06-12/qwages","a")
		fputmatrix(fh,qwages)
		fclose(fh)
		fh=fopen("${results}06-12/qhours","a")
		fputmatrix(fh,qhours)
		fclose(fh)
		fh=fopen("${results}06-12/pwages","a")
		fputmatrix(fh,pwages)
		fclose(fh)
		fh=fopen("${results}06-12/phours","a")
...
if (gender=="women"){
	if (B=="Yes"){
		fh=fopen("${results}w06-12/qwages","r")
		qwages=fgetmatrix(fh)
		fclose(fh)
		fh=fopen("${results}w06-12/qhours","r")
		qhours=fgetmatrix(fh)
		fclose(fh)
	}
	else{
		...
		pwages= mm_quantile(wages,weights,(.1,.2,.3,.4,.5,.6,.7,.8,.9))'
		phours= mm_quantile(hours,weights,(.1,.2,.3,.4,.5,.6,.7,.8,.9))'
		fh=fopen("${results}w06-12/qwages","a")
		fputmatrix(fh,qwages)
		fclose(fh)
		fh=fopen("${results}w06-12/qhours","a")
		fputmatrix(fh,qhours)
		fclose(fh)
		fh=fopen("${results}w06-12/pwages","a")
		fputmatrix(fh,pwages)
		fclose(fh)
		fh=fopen("${results}w06-12/phours","a")
		fputmatrix(fh,phours)
		fclose(fh)
	}
}
...
fh=fopen("${results}06-12/diff","a")
fputmatrix(fh,diff)
fclose(fh)

end

*Bootstrap (50 but can change)
preserve
forvalues i=1/50{
	global foo `i'
	set seed `i'
	...
	mata fh=fopen("${results}06-12/boot`i'","a")
...
fh=fopen("${results}w06-12/wdiff","a")
fputmatrix(fh,wdiff)
fclose(fh)
end
...
forvalues i=1/50{
	...
	mata fh=fopen("${results}w06-12/w_boot`i'","a")
	mata fputmatrix(fh,temp)
	mata fclose(fh)
	dis `i'
	restore, preserve
